            ...
            <tr className="bg-slate-50 dark:bg-slate-800/80 text-slate-800 dark:text-slate-200 font-bold border-b border-slate-200 dark:border-slate-700">
              ...
              {activeDays.map(day => (
                <th 
                  ...
                  title={userRole === 'admin' ? `${day.dateStr} (${day.dayOfWeek}) - 클릭 시 미체크 빈칸을 'X'(결석)으로 채웁니다` : `${day.dateStr} (${day.dayOfWeek})`}
                >
                  {userRole === 'admin' ? (
                    <button
                      onClick={() => onFillDayAbsent(day.dateStr, selectedGrade === 'all' ? undefined : Number(selectedGrade))}
                      className="w-full py-1 rounded-lg hover:bg-rose-100 dark:hover:bg-rose-950/60 font-black text-slate-900 dark:text-slate-100 hover:text-rose-600 transition-colors flex flex-col items-center justify-center cursor-pointer group"
                      title={`${day.dayNum}일: 클릭하면 미체크 빈칸을 'X'(결석)으로 채웁니다`}
                    >
                      <span>{day.dayNum}</span>
                      <span className="text-3xs text-rose-500 opacity-60 group-hover:opacity-100 font-bold leading-none">X</span>
                    </button>
                  ) : (
                        ...
                        {activeDays.map(day => {
                          ...
                          return (
                            <td
                              ...
                              className={`py-1 px-0.5 border-r border-slate-200 dark:border-slate-800 cursor-pointer select-none transition-colors font-bold text-sm ${meta.cellClass} ${meta.bgHover}`}
                              title={

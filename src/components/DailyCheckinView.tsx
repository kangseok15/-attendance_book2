            ...
            {userRole === 'admin' ? (
              <div className="space-y-2">
                <button
                  ...
                  className="w-full py-2.5 px-3 rounded-xl bg-rose-600 hover:bg-rose-500 active:bg-rose-700 text-white text-xs font-bold transition-all flex items-center justify-center gap-2 shadow-xs cursor-pointer"
                ...
                {(['PRESENT', 'LATE', 'EARLY_LEAVE', 'EXCUSED', 'ABSENT'] as AttendanceStatus[]).map(st => {
                  ...
                  return (
                    <button
                      ...
                      } ${userRole === 'teacher' ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer'}`}
                      title={userRole === 'teacher' ? '담임 교사는 조회 전용입니다' : `${student.name}: ${m.label} 체크`}
                    >

      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* Admin only: Cloud Sync (Google Sheets) Settings */}
            {userRole === 'admin' && onOpenSyncModal && (
              <button
                onClick={onOpenSyncModal}
                className={`p-2 rounded-xl transition-colors border shadow-2xs shrink-0 cursor-pointer ${
                  isSyncEnabled
                    ? 'text-emerald-600 border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/30 hover:bg-emerald-100'
                    : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-700'
                }`}
                title={isSyncEnabled ? '여러 기기 동기화 켜짐 (Google 스프레드시트)' : '여러 기기 동기화 설정 (Google 스프레드시트)'}
              >
                {syncStatus === 'syncing' ? (

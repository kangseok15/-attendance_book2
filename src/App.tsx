...
import { getRecordKey, isStudentExcluded, isStudentExcludedOnDate, sortStudents } from './utils/attendanceHelpers';
import { getSyncUrl, isSyncEnabled, pullFromCloud, pushToCloud } from './utils/cloudSync';
...
export default function App() {
  ...
  // Cloud sync (Google Sheets via Apps Script) state
  const [isSyncModalOpen, setIsSyncModalOpen] = useState(false);
  const [syncUrl, setSyncUrl] = useState<string>(() => getSyncUrl());
  ...
  // Pull the shared data from Google Sheets once on load (and whenever the sync URL changes)
  useEffect(() => {
    if (!syncUrl) return;
    let cancelled = false;
    (async () => {
      ...
      if (!cancelled && cloud) {
        setStudents(sortStudents(cloud.students, [3, 2, 1], true));
        setRecords(cloud.records);
        setLastSyncedAt(cloud.updatedAt || new Date().toISOString());
        setSyncStatus('idle');
      } else if (!cancelled) {
        setSyncStatus(cloud === null ? 'error' : 'idle');
      }
    })();
    return () => {
      cancelled = true;
  ...
  // Poll for updates from other devices every 20 seconds, and on tab focus
  useEffect(() => {
    if (!syncUrl) return;
  ...
  // Push local changes up to Google Sheets, debounced so rapid edits don't spam the network
  useEffect(() => {
    if (!syncUrl) return;
    const timer = setTimeout(async () => {
      const ok = await pushToCloud({ students, records });
      if (ok) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
      ...
      {/* Cloud Sync (Google Sheets) Settings Modal */}
      <SyncSettingsModal
        isOpen={isSyncModalOpen}
        onClose={() => setIsSyncModalOpen(false)}
        onSaveUrl={(url) => setSyncUrl(url)}
